...
`docs/RUNBOOK.md` — операционная инструкция: нормальный деплой, диагностика
«сайт не открывается», лестница восстановления (Restart Proxy → Restart App →
Rollback), какие логи снимать при рецидиве, мониторинг. При инциденте идти
туда, а не импровизировать. Внешний uptime-контроль —
`.github/workflows/uptime.yml` (независим от сервера, где живёт Coolify).
...
| 3 | Собственная AI-анкета + RF-hosted lead API; автоматический PDF не входит | ✅ код и проверки 2026-07-16, deploy отдельно |
| 4 | Деплой на `185.221.213.104` (Coolify/Traefik + Docker/Caddy) | ✅ 2026-07-16 |
