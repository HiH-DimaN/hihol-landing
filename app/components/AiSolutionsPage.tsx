  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-[var(--bg)]/95 backdrop-blur-sm">
      <nav className="mx-auto flex h-16 max-w-[1080px] items-center justify-between px-4 sm:px-6">
        <Link href="/" className="text-lg font-bold tracking-tight text-[var(--text)]">HIHOL</Link>
        <Link href="/" className="text-sm text-[var(--text-muted)] transition-colors hover:text-[var(--text)]">
          Проверка 152-ФЗ
        </Link>
      </nav>
    </header>
  ...
  return (
    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {AI_PAGE.tiles.map((tile) => (
        <div key={tile.title} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
          <h2 className="text-base font-semibold text-[var(--text)]">{tile.title}</h2>
          <p className="mt-1 text-sm leading-snug text-[var(--text-muted)]">{tile.line}</p>
        </div>
      ))}
    </div>
  )
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        {AI_PAGE.how.steps.map((step, i) => (
          <div key={step} className="flex items-center gap-3">
            ...
            {i < AI_PAGE.how.steps.length - 1 && <span className="text-[var(--accent)]">-&gt;</span>}
          </div>
        ))}
      </div>
    </div>
...
function AiStack() {
  return (
    <div className="mt-12 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
      <h2 className="text-xl font-semibold text-[var(--text)] sm:text-2xl">{AI_PAGE.stack.title}</h2>
      <p className="mt-3 max-w-2xl leading-relaxed text-[var(--text-muted)]">{AI_PAGE.stack.text}</p>
    </div>
  )
}
...
function AiBridge() {
  return (
    <p className="mt-12 border-t border-[var(--border)] pt-6 text-sm text-[var(--text-muted)]">
      {AI_PAGE.bridge} -{' '}
      <Link href="/" className="font-medium text-[var(--accent)] underline underline-offset-2 hover:text-[var(--accent-strong)]">
        перейти на главную
      </Link>
    </p>
  )
