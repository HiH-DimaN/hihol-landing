...
const cases: Case[] = [
  {
    ...
    task: 'Потеря лидов 38% из-за долгого ответа (15 мин). Типовые вопросы по доставке, размерам и остаткам заваливают менеджеров.',
    solution:
      'AI-чат на Vercel Serverless + OpenAI GPT-4o-mini. Типовые вопросы закрывает бот, редкие эскалирует на менеджера.',
    result: 'Выручка +31%, ответ 15 мин → 40 сек, потеря лидов 38% → 7%. ROI — 12 дней.',
    duration: '10 дней',
  },
  {
    emoji: '🎓',
  ...
  },
  {
    ...
    task: 'Потолок 60–80 сделок/мес. Скорость ответа 5–10 мин — лиды уходят к конкурентам. Потеря лидов 32%.',
    solution:
      'TMA с каталогом квартир, 3D-туры, AI-чат с семантическим поиском через PG Vector, дашборд агентства.',
    result: 'Сделки 60–80 → 95–110/мес (+38%), ответ 5–10 мин → 45 сек, потеря лидов 32% → 9%. ROI 3–4 недели.',
    duration: '3 недели',
  },
]
  ...
  return (
    <article
      className={`group relative flex flex-col gap-5 overflow-hidden rounded-2xl border p-6 transition-all duration-300 ${
        c.muted
          ? 'border-dashed border-stone-700/80 bg-[#151812]/40'
          : 'border-stone-800/80 bg-[#151812]/70 backdrop-blur-sm hover:-translate-y-1 hover:border-amber-300/40 hover:shadow-[0_0_40px_-16px_rgba(245,158,11,0.42)]'
      }`}
    >
      {!c.muted && (
        <div
          aria-hidden="true"
          className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-amber-500/10 blur-2xl transition-colors duration-300 group-hover:bg-emerald-500/20"
        />
      )}
      ...
      <div
        ...
        className={`relative flex h-14 w-14 items-center justify-center rounded-2xl text-3xl ${
          c.muted
            ? 'border border-dashed border-stone-700 bg-[#151812]/70'
      ...
      <h3
        className={`relative text-xl font-bold ${
          c.muted ? 'text-stone-400' : 'text-white'
        }`}
      >
        {c.title}
      </h3>
        ...
        <div>
          ...
          <dd
            className={`mt-1 leading-relaxed ${
              c.muted ? 'text-stone-400' : 'text-stone-300'
            }`}
          >
            {c.task}
          </dd>
        ...
        <div>
          ...
          <dd
            className={`mt-1 font-medium leading-relaxed ${
              c.muted ? 'text-stone-400' : 'text-amber-200'
            }`}
          >
            {c.result}
          </dd>
      ...
      {c.duration && (
        <div className="relative mt-auto flex items-center gap-2 border-t border-stone-800/80 pt-4 text-xs text-stone-500">
          <ClockIcon />
          Срок: {c.duration}
        </div>
      )}
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-16 text-center">
          ...
          <p className="mt-4 text-lg text-stone-300">
            Реальные проекты, данные обезличены
          </p>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-stone-500">
            Сферы, цифры и сценарии адаптированы так, чтобы показать экономику
            проекта без раскрытия клиентских данных.
          </p>
        </div>
